...
如果您发送 `.截图` 命令后收到"没有已连接的桌面客户端"错误，请按以下顺序检查：

1. **发送诊断命令**：在 QQ 中发送 `.桌面状态` 或 `.desktop_status` 查看当前连接状态
2. **检查服务端日志**：查找 `✅ WebSocket 服务器已启动` 字样
3. **检查桌面客户端**：确认桌面客户端已启动并配置了正确的服务器地址
4. **检查网络**：确保防火墙已开放 6190 端口
