...
class ScreenCaptureService:
    ...
    def __init__(self, save_dir: str = "./temp/screenshots"):
        ...
        if not HAS_MSS:
            raise ImportError("mss 库未安装，请执行: pip install mss")
        if not HAS_PIL:
            raise ImportError("Pillow 库未安装，请执行: pip install Pillow")
    ...
    def capture_full_screen_to_file(self, filename: Optional[str] = None) -> Optional[str]:
        """
        捕获全屏并保存到文件
